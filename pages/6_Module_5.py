...
def load_module_data():
    ...
    try:
        ...
        if "modules" in data and isinstance(data["modules"], list):
            ...
            target_title = "Large Kit: Reactors and Reaction Kinetics" # Corrected title
...
def main():
    ...
    render_tutor_interface(
        module_id="5",
        module_title=module_data.get("title", "Large Kit - Reactors"), # Corrected default title
        module_description=module_data.get("description", ""),
        topics=module_data.get("topics", []),
        file_id=st.session_state.get("module_5_file_id")
    )
